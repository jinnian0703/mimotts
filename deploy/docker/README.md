...
首次打开安装页填写管理员账号密码。Docker 会自动准备 APP_KEY、SQLite 数据库、迁移和运行目录。
...
Docker 部署仍通过安装页创建管理员。容器会自动创建 SQLite 数据库、执行迁移，并把 `.env` 里的 Mimo API、LinuxDo、邮箱登录开关同步到系统配置。`.env` 保持示例 `APP_KEY` 时，容器会自动生成并保存到 Docker 数据卷中。
